...
if [ ! -f "$file_to_check" ]; then
  echo "DataStreamPilot: ELF NOT FOUND"
...
if [ -n "$IOT_LAB_FRONTEND_FQDN" ]; then
  echo "DataStreamPilot: Copy firmware files to shared"
  ...
  echo "DataStreamPilot: Flashing new firmware for ${my_arch} node : ${COAP_SERVER_NODE}"
