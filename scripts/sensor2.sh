...
SENSOR_2_CONNECTED_NODE=${SENSOR_2}
build_wireless_firmware ${SENSOR_2_CONNECTED_HOME} ${SENSOR_2_CONNECTED_EXE_NAME}
build_status=$?
...
if [ -n "$IOT_LAB_FRONTEND_FQDN" ]; then
  cp ${SENSOR_2_CONNECTED_HOME}/bin/${ARCH}/${SENSOR_2_CONNECTED_EXE_NAME}.elf ${SENSE_FIRMWARE_HOME}

  iotlab-profile del -n group12
  iotlab-profile addm3 -n group12 -voltage -current -power -period 8244 -avg 4

  n_json=$(iotlab-experiment submit -n ${SENSOR_2_CONNECTED_EXE_NAME} -d 20 -l ${SENSE_SITE},m3,${SENSOR_2_CONNECTED_NODE},${SENSE_FIRMWARE_HOME}/${SENSOR_2_CONNECTED_EXE_NAME}.elf,group12)
  ...
  echo "nc m3-${SENSOR_2_CONNECTED_NODE} 20000"
  nc m3-${SENSOR_2_CONNECTED_NODE} 20000
