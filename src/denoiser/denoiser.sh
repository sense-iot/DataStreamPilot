...
make BOARD=${ARCH}
echo bin/${ARCH}/denoiser_gp12.elf

if [ -n "$IOT_LAB_FRONTEND_FQDN" ]; then
  cp bin/${ARCH}/denoiser_gp12.elf ~/shared/
  ...
  n_json=$(iotlab-experiment submit -n denoiser-g12 -d 3 -l grenoble,m3,356,~/shared/denoiser_gp12.elf,group12)
